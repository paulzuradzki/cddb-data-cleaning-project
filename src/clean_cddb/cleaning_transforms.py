...
The idea is that each "clean_df*()" function takes a dataframe and returns a dataframe after applying a cleaning procedure.
...
def clean_df_standardize_various_artists(df) -> pd.DataFrame:
    return df.assign(artist=lambda _df: _df['artist'].apply(clean_value_standardize_various_artist))
