...
def get_check_func_descriptions(failure_cases_df, schema):
      ...
      failure_cases_df_with_source = (
      failure_cases_df
      .assign(check_source_code=lambda _df: _df['check'].map(check_name_and_descriptions)
              )
            )
...
def display_failure_cases_summary(failure_cases_df):    
    ...
    failure_cases_df
    .groupby(["column", "check", "check_source_code"], as_index=False)
    .size()
    ...
    formatted_table:str = tabulate.tabulate(report_items, headers='keys', tablefmt='grid')
